...
void *worker(void *arg)
{
	...
	// === init vm
	dev_write(vm_fd, "function \"current:dc\"");
	dev_write(vm_fd, "current:dc:range:auto on");
	dev_write(vm_fd, "current:dc:nplcycles 10");
	...
	// === write vac header
	fprintf(vac_fp, "# Classical VAC experiment with laser on\n");
	...
	while(get_run())
	{
		...
		fprintf(vac_fp, "%d\t%le\t%.3le\t%.3le\t%.8le\t%.3le\t%.3le\n",
			vac_index, vac_time, pps_voltage, pps_current,
			vm_current, laser_voltage, laser_current
		);

		fprintf(gp,
			"set title \"i = %d, t = %.3lf s, Ul = %.3lf V, Il = %.3lf A\"\n"
			"plot \"%s\" u 3:5 w l lw 1 title \"U = %.3lf V, I = %.3le A\"\n",
			vac_index, vac_time, laser_voltage, laser_current,
			filename_vac, pps_voltage, vm_current
		);
